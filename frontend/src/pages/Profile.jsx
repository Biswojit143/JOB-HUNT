...
const Profile = () => {
  ...
  {
    (() => {
      console.log("Applied jobs:", appliedJobs); // Debugging log

      return appliedJobs
        ?.filter(ap => ap && ap.job && ap.job.title && ap.job.company) // Defensive check
        .map((ap) => {
          console.log("Individual job entry:", ap); // Log each job row
          ...
          return (
            <tr key={ap._id}>
              ...
              <td className="px-4 py-3">
                <span className="text-sm bg-gray-800 text-white rounded-full px-3 py-0.5">
                  {ap.status}
                </span>
              </td>
            </tr>
